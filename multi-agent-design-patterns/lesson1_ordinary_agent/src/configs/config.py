...
class Config(BaseModel):
    ...
    enable_content_guard_llm: bool = Field(
        default=False, description="是否启用LLM内容审查"
    )
    ...
    def _load_model_names(self, model_names_data):
        ...
        try:
            ...
            logger.info(
                f"Loaded custom model configurations for {len(model_names_data)} providers"
            )
        except Exception as e:
    ...
    def dump_config(self) -> dict[str, Any]:
        ...
        config_dict["embed_model_names"] = {
            ...
            for model_id, info in self.embed_model_names.items()
        }
        ...
        for field_name, field_info in Config.model_fields.items():
            if not field_info.exclude:  # 排除内部字段
                fields_info[field_name] = {
                    ...
                    "exclude": (
                        field_info.exclude if hasattr(field_info, "exclude") else False
                    ),
                }
        ...
        try:
            ...
            if provider_name:
                ...
                if provider_name in self.model_names:
                    user_config["model_names"][provider_name] = self.model_names[
                        provider_name
                    ].model_dump()
                    # 记录具体修改的 provider
        ...
        try:
            ...
            if custom_providers:
                custom_config["model_names"] = {
                    provider: info.model_dump()
                    for provider, info in custom_providers.items()
                }
